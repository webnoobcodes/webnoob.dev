<template>
  <div class="socials">
    <div class="platform" v-for="(item, index) in platforms" :key="'platform'+index">
      <a :href="item.link" target="_blank"><fa-icon :icon="['fab', item.icon]" size="3x" :style="{'color': item.color}" /></a>
      <div>{{ item.title }}</div>
    </div>
  </div>
</template>
